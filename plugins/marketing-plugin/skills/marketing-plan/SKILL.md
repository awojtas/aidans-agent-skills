...
Synthesises everything upstream into a plan someone with very little time can actually follow.

## When to use

After positioning. Re-run when constraints, channels, or results change materially.
...
`marketing-plan.md` — the source of truth that `marketing-assets`, `marketing-launch`, and `marketing-metrics` execute against.

## Lifecycle tracker

This skill owns the **Plan** stage of the marketing lifecycle tracker kept at the bottom of the acted-on repo's `README.md`. See [`../../shared/marketing-lifecycle.md`](../../shared/marketing-lifecycle.md) for the block format, emoji legend, and create-or-update algorithm.

- **When this skill begins its substantive work** (after prerequisites pass), set the `Plan` line to ⏳ (in progress). Create `README.md` and/or the tracker block first if either is missing.
- **When this skill completes successfully**, set the `Plan` line to ✅ (done).

Touch only the `Plan` line — leave every other stage exactly as found.
