...
4. **Wiring checks:**
   ...
   - **Cross-origin API reachability (when architecture has separate frontend + API on different origins):**
     - (a) **No platform SSO gate on the API:** fetch a public endpoint (e.g., `/api/health`) and confirm the response is not a redirect to a platform login page (no 302 to a Vercel auth URL, no `www-authenticate: Bearer realm="vercel"`). It should return 200 or an app-level response. Flag as **red** — end-user browsers cannot authenticate through a platform SSO gate.
     - (b) **Protected endpoints return the app's own 401:** fetch a protected endpoint without a token and confirm it returns the app's own `401` (or auth error), not a platform authentication challenge. Flag as **red** if the platform intercepts first.
     - (c) **Cross-origin preflight succeeds:** send an `OPTIONS` request to an API endpoint with `Origin: <client-url>` and `Access-Control-Request-Headers: Authorization`. Confirm the response includes `Access-Control-Allow-Origin: <client-url>` and `Access-Control-Allow-Headers: authorization`. Flag as **red** if absent or incorrect — all authenticated browser requests will be blocked by CORS.
     - (d) **Client's API-URL env var is set:** confirm the frontend project's env store contains the API base-URL var (e.g., `NEXT_PUBLIC_API_URL`). Flag as **red** if missing — the client will call `undefined/api/...` at runtime.
