---
name: ai-ready-repo
description: 'Audits an existing mature codebase for AI-readiness — whether an AI agent (Claude Code, Copilot, Cursor) can work effectively or will trip on layout/convention/landmine issues invisible to humans. Checks agent-instruction hygiene (AGENTS.md, lean CLAUDE.md, copilot-instructions), layout & stack legibility, and confusion landmines (stale comments, misleading names, hidden re-exports). Reports tiered findings, applies approved fixes in a PR, and stamps a dated AI-readiness marker into README.md. For mature repos that never went through /repo-bootstrap. Use when the user says "is this repo AI-ready", "AI readiness check", "make this repo AI-friendly", "agent-readiness audit", or "can Claude work in this repo".'
---
