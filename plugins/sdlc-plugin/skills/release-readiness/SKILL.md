...
description: Per-feature production-readiness check before a change merges or deploys. Reads the diff (or a PR), walks the operational checklist — observability (logs / metrics / traces on the new code path), alerting (new failure modes covered), runbook + rollback plan, capacity + cost sanity, failure modes (timeouts / retries / graceful degradation), deploy safety (env vars in all envs, canary compatibility) — and produces a green / red verdict with specific gaps. **Per-feature scope**, distinct from `/platform-verify` which audits platform-level provisioning. Use when the user says "ready to ship", "release readiness", "is this ready for prod", "production readiness check", "pre-launch check", "operability review", "SRE pass on this", or wants a focused production-readiness assessment of a specific change. Companion to `/pr-review` (which checks code quality) — `/release-readiness` checks ops-readiness.
---
