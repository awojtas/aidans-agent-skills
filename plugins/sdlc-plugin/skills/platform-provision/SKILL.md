...
4. **Provision what you can autonomously.** Create the project / org / dashboard / DB / whatever. Capture every output the human will care about later: resource IDs, regions, URLs, DSNs, project tokens, organization slugs. Record them as you go.
   ...
   **Serverless adapter entrypoint (standalone API frameworks):** a Next.js app deploys natively on Vercel. A standalone API framework (Hono, Express, Fastify, etc.) requires a serverless adapter entrypoint — a specific file (e.g., `api/index.ts`, a `vercel.json` `functions` config) that wraps the app to run as serverless functions. Verify this file exists and is correctly configured before considering the app provisioned. Checking that the framework is imported is not enough — the entrypoint must exist. Three Vercel-specific pitfalls for non-Next.js backends: (1) **Framework Preset** — set the Vercel project's Framework Preset to "Other"; auto-detect may apply a Next.js preset and silently override the build command and output directory. (2) **ESM vs CJS** — the bundled output format and the project's `"type"` field in `package.json` must agree; a mismatch causes silent cold-start failures. (3) **Monorepo Root Directory** — in a monorepo, set the Vercel project's Root Directory to the backend app's subdirectory (e.g. `apps/api`); without it, Vercel builds from the repo root and may not locate the entrypoint.
