...
description: 'Audits an application''s whole security posture across solution, architecture, platform, coding, and data storage — mapped to OWASP Top 10:2025 and Secure-by-Design. Detects the stack, runs available scanners (dependency/supply-chain audit, secret scanning, SAST) as evidence, reviews code, dependencies, secrets hygiene, auth, and data-at-rest, then reports tiered P0/P1/P2 findings with file:line evidence. Read-only — never edits. Use when the user says "check my app''s security", "security posture audit", "is my app secure", "OWASP audit", "secure-by-design review", "secrets/dependency audit", or "application security review". For a diff-scoped review use /security-review; for platform wiring use /platform-verify.'
---
