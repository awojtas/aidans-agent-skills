...
4. **Wiring checks:**
   ...
   - **Framework Preset (Vercel, non-Next.js backends):** for any standalone backend (Hono, Express, Fastify, etc.) deployed as a Vercel project, confirm the project's Framework Preset is set to "Other" (or the correct value) — not auto-detected. A wrong preset silently overrides the build command and output directory, deploying a different artifact than expected. Check Vercel dashboard → project Settings → General → Framework Preset. Flag as **red** if it's set to a framework the app isn't using.
   ...
   - **Web app ↔ API wiring (when architecture has a separate web frontend + API as separate Vercel projects):** verify the BFF + Trusted Sources pattern is in place:
     - (a) **API Deployment Protection is ON:** confirm the API project has Deployment Protection enabled. A direct unauthenticated GET to the API URL should return a Vercel authentication challenge (not a 200). Flag as **red** if protection is off — the API is exposed to the public internet.
     - (b) **Trusted Sources configured on the API project:** verify the web app's Vercel project appears as a trusted source (Settings → Deployment Protection → Trusted Sources). Flag as **red** if absent — BFF calls from the web app will be blocked.
     - (c) **BFF proxy route handlers exist in the web app:** check the repo for proxy handler code (e.g. `app/api/[...path]/route.ts`) that imports `getVercelOidcToken` from `@vercel/oidc` and adds the `x-vercel-trusted-oidc-idp-token` header to outgoing API calls. Flag as **red** if missing.
     - (d) **`API_URL` server-side env var set on the web app project:** confirm the web app's Vercel project env store has `API_URL` (or equivalent) pointing to the API's deployment URL. It must be a server-side var (not `NEXT_PUBLIC_`). Flag as **red** if missing.
     - (e) **No CORS check needed** — the browser only calls its own origin (the web app). Do not flag absent CORS headers on the API as an issue.
