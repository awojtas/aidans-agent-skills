...
This is the **mobile** entry point to the `/repro-visual` skill. Follow `repro-visual`'s full workflow, with these defaults:
...
- Pay attention to mobile-only mechanics: horizontal-scroll containers (`min-width` + `overflow-x`), `position: sticky` columns overlaying content, touch targets, and the on-open scroll/centre.
