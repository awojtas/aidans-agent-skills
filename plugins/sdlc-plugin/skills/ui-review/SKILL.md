---
name: ui-review
description: 'Opinionated, evidence-based review of a running UI''s visual/UX quality, around the question "deliberate design choice, or leftover tool default?" Must see the rendered app (chrome-devtools MCP or the repro-visual harness; consent-gated static-only fallback), measures computed styles (contrast, type scale, spacing) rather than eyeballing, checks states across breakpoints and dark mode, runs an anti-slop pre-scan, and ends in a ruthless Critical/High/Medium/Low action list. Read-only. Use when the user says "UI review", "design review", "review the UI", "does this look AI-generated", "design QA", or "why does my app look generic". Not component reuse (ui-component-review) or creating UI (frontend-design).'
---
