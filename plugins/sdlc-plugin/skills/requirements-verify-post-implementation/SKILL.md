...
description: 'Post-implementation verification — checks the actually-shipped code and tests against every requirement in docs/requirements/, producing a traceability report marking each FR/NFR Met / Partial / Not met / No evidence. The whole-system closing QA sweep: catches requirements never turned into an issue, NFRs no single ticket delivered, and spec-vs-build drift. Raises GitHub issues for gaps; never silently fixes them. Distinct from /requirements-validation (pre-build) and the per-issue check in /task-implement. Use when the user says "verify the requirements are implemented", "did we build everything", "requirements traceability", "post-implementation verification", "requirements coverage check", or "V&V pass".'
---
