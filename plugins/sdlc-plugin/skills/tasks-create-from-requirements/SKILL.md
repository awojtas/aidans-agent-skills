...
description: 'Turns the requirements in docs/requirements/ into a concrete GitHub-issue implementation plan. Decomposes each requirement into small-batch tasks (<=1 day), stages them with numbered milestones, isolates human-required work (accounts, secrets, decisions) into a Phase 0 milestone, and writes issues with a clear Definition of Done plus Given-When-Then acceptance criteria. Shows the plan for approval before any GitHub mutations. Use when the user says "plan the implementation", "create issues from requirements", "break this down into tasks", "what do we build first", "make a backlog", or "issuify the requirements".'
---
...
**Cross-cutting tasks (UI/web projects):** after decomposing feature requirements, check for cross-cutting tasks that are consistently missing from decomposed plans — see the full rules in `references/task-decomposition-guide.md` (section 3):
...
3. **Web app ↔ API integration (BFF + Trusted Sources)** — if the architecture has separate web and API Vercel projects, two integration tasks must exist: a Phase 0 `human-required` task for Trusted Sources configuration (dashboard-only, no CLI), and an AI delivery task for the BFF proxy implementation that carries `Blocked by:` the Phase 0 task. See `references/task-decomposition-guide.md` (section 3) for the full split.
