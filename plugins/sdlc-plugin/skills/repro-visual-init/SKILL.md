...
description: 'One-time setup that scaffolds the visual-repro harness into a repo so /repro-visual can drive the deployed app in an emulated browser. Inspects this repo to wire the per-project glue — base URL, login flow + selectors, auth mechanism (Supabase/cookie/SSO/Cognito/Entra/IAP/none), the API used to seed test data, and the selectors + metrics that define "correct" — then writes scripts/repro/ (Playwright harness with cached-session login), a gitignored .env.e2e, a package.json "repro" script, and an AGENTS.md pointer. Run once per repo, or to refresh an existing harness. Triggers: set up the repro harness, init repro-visual, scaffold the visual repro tooling, add the mobile repro harness.'
---

Scaffold the **visual-repro harness** into the current repo and wire it to *this* project's specifics, so `/repro-visual` (and `/repro-on-mobile` / `/repro-on-desktop-web`) can reproduce and verify layout bugs against the deployed app. Run once per repo.
