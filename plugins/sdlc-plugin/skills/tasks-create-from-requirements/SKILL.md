...
3. **Client ↔ API integration** — if the architecture has a separate client + API on different origins, an explicit integration task must exist in the first phase that wires the two together. The failure mode: each app passes its own tests and deploys successfully, but the product fails at runtime — CORS blocks the browser, the env var is unset, or the API sits behind a platform SSO wall that real users can't get through.
