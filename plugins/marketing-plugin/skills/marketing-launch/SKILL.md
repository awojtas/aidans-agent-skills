...
description: 'Run the actual promotion as per-channel runbooks — directory submissions, a Product Hunt launch, value-first Reddit/forum posts, LinkedIn. The agent PREPARES everything; the human posts anything public. Each push is logged in the growth tracker. Use when the user says "launch", "Product Hunt", "submit to directories", "post to Reddit", or is ready to start promoting.'
---
...
- **Directories (seed):** prep every listing (description, links, screenshots) as a copy-paste pack; human submits. One-time, permanent backlinks.
