...
description: 'Removes or demotes a single requirement from an existing docs/requirements/ tree, with a full cascade-impact scan first. Asks whether the intent is deletion or demotion (lower MoSCoW, or Won''t), scans every requirement/assumption/open-question/fit-criterion/traces-to chain for references so nothing silently breaks, surfaces any GitHub issues that implement it, and presents the full change set for approval before anything destructive. For one requirement, not plan-wide pivots (use /requirements-rework). Trigger phrases: "delete this requirement", "remove FR-X", "we don''t need this requirement anymore", "drop this requirement", "demote this requirement", "lower the priority of this requirement".'
---
