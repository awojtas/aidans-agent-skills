...
3. **Web app ↔ API integration (BFF + Trusted Sources)** — if the architecture has separate web and API Vercel projects, an explicit integration task must exist covering: BFF proxy route handlers in the web app, Trusted Sources configured on the API project, `API_URL` server-side env var on the web app, and a real end-to-end call through user auth as the acceptance criterion. The failure mode: each deploys successfully, but the product fails at runtime because the BFF was never wired, the OIDC token is missing, or the env var is unset.
